<#
    ...
    Pré-requisitos:
      ...
      - O Waitress deve rodar em 127.0.0.1:8000 (ver iniciar.bat / serviço NSSM;
        no .env: MCMV_HOST=127.0.0.1  MCMV_PORT=8000).
      - Após o apontamento de DNS de mcmv.baraodecocais.mg.gov.br -> 172.16.64.9.
    ...
    Para baixar/instalar automaticamente os módulos URL Rewrite e ARR (requer
...
if ((-not $temRewrite -or -not $temArr) -and $BaixarModulos) {
    ...
    if (-not $temRewrite) {
        ...
        Write-Host "  Baixando URL Rewrite..."; Invoke-WebRequest $u -OutFile $f
        Start-Process msiexec.exe -ArgumentList "/i `"$f`" /qn" -Wait
    }
    if (-not $temArr) {
        ...
        Write-Host "  Baixando ARR..."; Invoke-WebRequest $u -OutFile $f
        Start-Process msiexec.exe -ArgumentList "/i `"$f`" /qn" -Wait
    }
    $temRewrite = [bool](Get-WebGlobalModule | Where-Object { $_.Name -eq "RewriteModule" })
    $temArr = Test-Path $arrDll
...
if (-not $temRewrite -or -not $temArr) {
    Write-Host "FALTAM módulos do IIS:" -ForegroundColor Yellow
...
if ($ProjetoDir -and (Test-Path (Join-Path $ProjetoDir ".env"))) {
    ...
    Write-Host "  Reinicie o Waitress (serviço MCMV ou iniciar.bat) para aplicar." -ForegroundColor Yellow
} else {
...
Write-Host ""
Write-Host "Concluído. Após o DNS apontar $Hostname -> 172.16.64.9, acesse:" -ForegroundColor Green
Write-Host "  http://$Hostname" -ForegroundColor Green
Write-Host "Garanta que o Waitress esteja rodando em $Backend (serviço MCMV/NSSM)." -ForegroundColor Green
